...
public class ProgressBar extends View {
    ...
    // 滑竿的内部小圈颜色 #181718
    public static final int DEFAULT_CIRCLE_COLOR_IN = Color.parseColor("#181718");
    ...
    public static final float DEFAULT_STROKE_WIDTH = Resources.getSystem().getDisplayMetrics().density * 1.5f;
    ...
    public void setActiveLineColor(int activeLineColor) {
        this.mActiveLineColor = mContext.getResources().getColor(activeLineColor);
    }
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        showProgressText = ta.getBoolean(R.styleable.ProgressBar_showProgressText, true);
        ta.recycle();
    }
    ...
    private void initSize(int w, int h) {
        ...
        mLinePosition = h - mCircleRadius - getPaddingTop();
    }
    ...
    private void drawLine(Canvas canvas) {
        ...
        if (mNegativeable) {
            ...
            if (mProgress < 0.5) {
                ...
                endX = mLeftPadding + (mWidth - mLeftPadding - mRightPadding) * 0.5f - mCircleRadius;
            } else {
                startX = mLeftPadding + (mWidth - mLeftPadding - mRightPadding) * 0.5f + mCircleRadius;
            ...
            paint.setColor(mActiveLineColor);
            startX = mLeftPadding;
            endX = mLeftPadding + (mWidth - mLeftPadding - mRightPadding) * mProgress - mCircleRadius;
            canvas.drawLine(startX, startY, endX, endY, paint);

            paint.setColor(mInactiveLineColor);
            startX = endX + mCircleRadius * 2;
            endX = mWidth - mRightPadding;
            canvas.drawLine(startX, startY, endX, endY, paint);
        }
    }
...
//        paint.setStrokeWidth(mStrokeWidth);
        ...
        cy = mLinePosition - mTextHeight + metrics.bottom - textHeight / 2F;
        radius = textHeight / 2F + mTextPadding;
//        LogKit.d(Constant.TAG,"cx:" + cx + "  cy:" + cy + " radius:" + radius);
        canvas.drawCircle(cx, cy, radius, paint);
        paint.setColor(DEFAULT_CIRCLE_COLOR_IN);
...
//            LogKit.d(Constant.TAG,"cy1:" + cy);
    ...
    private void drawText1(Canvas canvas) {
        if (true) {
            ...
            float cx = mLeftPadding + (mWidth - mLeftPadding - mRightPadding) * mProgress;
//            float cy = mLinePosition - mTextHeight;
            float cy1 = cy - radius - 20;
//            LogKit.d(Constant.TAG,"cy1:" + cy1);
            canvas.drawText(text, cx, cy1, paint);
        }
    }
