...
class FloatSliderView @JvmOverloads constructor(
    ...
    private var showSubProgress = false
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.SliderView).apply {
            //当前进度进度条颜色（美团黄）
            lineColor = getColor(
                R.styleable.SliderView_lineDefColor,
            ...
            showSubProgress = getBoolean(R.styleable.SliderView_showSubProgress, false)
            recycle()
        }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event.action) {
            ACTION_DOWN -> {
                ...
                listener?.onBegin(position.toInt())
                onChange(position)
                isSliding = true
            ...
            ACTION_UP, ACTION_CANCEL -> {
                if (isSliding) {
                    ...
                    listener?.onFreeze(currPosition.toInt())
                }
    ...
    private fun onChange(position: Float) {
        if (currPosition != position) {
            ...
            listener?.onChange(currPosition.toInt())
        }
    }
    ...
    fun setRange(minValue: Int, maxValue: Int) {
        ...
        currPosition = min(this.maxValue, currPosition)
        currPosition = max(this.minValue, currPosition)
        requestLayout()
    }
