...
import kotlinx.android.synthetic.main.btm_panel_text_anim.*
import kotlin.math.min

class TextAnimFragment : BaseFragment() {
    ...
    private fun init() {
                ...
                override fun onActionUp(touchArea: MutexSeekBar.TouchArea, indicatorValue: Int) {
                    when (touchArea) {
                        MutexSeekBar.TouchArea.LEFT_INDICATOR -> {
                            ...
                            stickerViewModel.curSticker()?.animation?.let {
        ...
        bar_loop_duration.apply {
            setOnSliderChangeListener(object : OnSliderChangeListener() {
        ...
        recycler_animation_in.apply {
            setResourceListInitListener(object : ResourceListInitListener {
                override fun onResourceListInitFinish() {
                    ...
                    val animation = stickerViewModel.curSticker()?.animation
                    val inDuration = animation?.takeIf { !it.loop }?.inDuration
                    currInPosition = if (inDuration != null) inDuration / 1000 else DEFAULT_IN_DURATION
                    selectItem(animation?.takeIf { !it.loop }?.inAnim?.resourceFile ?: "")
                }
            })
            init(getResourceViewConfig(resourceConfig?.textAnimationResConfig?.categoryIn ?: DefaultResConfig.TEXT_ANIM_CATEGORY_IN))
        ...
        recycler_animation_out.apply {
            setResourceListInitListener(object : ResourceListInitListener {
                override fun onResourceListInitFinish() {
                    ...
                    currOutPosition = if (outDuration != null) outDuration / 1000 else DEFAULT_OUT_DURATION
                    selectItem(animation?.takeIf { !it.loop }?.outAnim?.resourceFile ?: "")
                }
            })
            init(getResourceViewConfig(resourceConfig?.textAnimationResConfig?.categoryOut ?: DefaultResConfig.TEXT_ANIM_CATEGORY_OUT))
            setOnItemClickListener(object : ResourceItemClickListener {
                override fun onItemClick(item: ResourceItem?, position: Int, select: Boolean) {
                    selectOutAnim(item)
                    selectItem(item?.path ?: "")
        ...
        recycler_animation_loop.apply {
            setResourceListInitListener(object : ResourceListInitListener {
                override fun onResourceListInitFinish() {
                    ...
                    currLoopPosition = if (loopDuration != null) loopDuration / 1000 else DEFAULT_LOOP_DURATION
                    selectItem(animation?.takeIf { it.loop }?.inAnim?.resourceFile ?: "")
                }
            })
            init(getResourceViewConfig(resourceConfig?.textAnimationResConfig?.categoryLoop ?: DefaultResConfig.TEXT_ANIM_CATEGORY_LOOP))
            setOnItemClickListener(object : ResourceItemClickListener {
                override fun onItemClick(item: ResourceItem?, position: Int, select: Boolean) {
                    selectLoopAnim(item)
                    selectItem(item?.path ?: "")
    ...
    private fun getResourceViewConfig(category: String) = ResourceViewConfig.Builder()
        .panelKey(resourceConfig?.textAnimationResConfig?.panel ?: DefaultResConfig.TEXT_ANIMATION)
        .hasCategory(true)
                ...
                item?.path?.takeIf { it.isNotEmpty() }?.let {
                    animation = animation?.apply {
                        ...
                        inAnim = NLEResourceNode().apply {
                            ...
                            resourceType = NLEResType.ANIMATION_TEXT
                        }
    ...
    private fun selectLoopAnim(item: ResourceItem?) {
        ...
        stickerViewModel.curTrackSlot()?.let { currSlot ->
            val slotDuration = (currSlot.duration / 1000f).toInt() // 微秒
            val loopMaxDuration = min(slotDuration, DEFAULT_MAX_LOOP_DURATION)
    ...
    private fun initTabByNLETrackSlot() {
        when (stickerViewModel.curSticker()?.animation.getAnimationType()) {
    ...
    private fun resetSeekBar() {
        ll_inout_layout.visibility = View.GONE
        ll_loop_layout.visibility = View.GONE
    }
    ...
    private fun updateUIByNLETrackSlot() {
        stickerViewModel.curTrackSlot()?.let {
            ...
            // 调整的时长最长不超过我们指定的 DEFAULT_MAX_LOOP_DURATION
            val loopMaxDuration = min(slotDuration, DEFAULT_MAX_LOOP_DURATION)
            // 设置可调整的时长范围
        ...
        val inDuration = stickerViewModel.curSticker()?.animation?.inDuration ?: 0
        val outDuration = stickerViewModel.curSticker()?.animation?.outDuration ?: 0

        val animationType = stickerViewModel.curSticker()?.animation.getAnimationType()
        ...
        when {
            ...
            (animationType == ANIMATION_TYPE_IN
                ...
                bar_inout_duration.setBothIndicator((inDuration / 1000f).toInt(), (outDuration / 1000f).toInt())
            }
        }
    }
