...
public class CircularProgressView extends View {
    ...
    private List<SegmentProgress> mSegmentProgressList = new ArrayList<>();
    private SegmentProgress mCurSegmentProgress = new SegmentProgress();
    private int mProgress = 0;
    private static final int DIVIDER_ANGLE = 2;
    ...
    public CircularProgressView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        LogUtils.d("CircularProgressView-------------");
...
//        @SuppressLint("Recycle")
        ...
        mProgPaint.setStrokeCap(Paint.Cap.BUTT);   // 设置方角
        ...
        // 初始化进度
        mCurSegmentProgress.setEndTime(typedArray.getInteger(R.styleable.CircularProgressView_progress1, 0));
        mSegmentProgressList.add(mCurSegmentProgress);
        typedArray.recycle();
...
//        canvas.drawArc(mRectF, 275, 360 * mProgress / maxDuration, false, mProgPaint);
        for (SegmentProgress segment : mSegmentProgressList) {
            long start = 360 * segment.startTime / maxDuration + 270;//初始角度为270
            long sweep = 360 * segment.endTime / maxDuration - start + 270;
            int dividerAngle = DIVIDER_ANGLE;
            if (segment.endTime >= maxDuration) {
                dividerAngle = 0;//full时不需要间隔
            }
            if (sweep > dividerAngle) {
                canvas.drawArc(
                        mRectF, start,
                        sweep - dividerAngle, false, mProgPaint
                );
            }
        }
//        setBackgroundResource(R.drawable.bt_view1);
    }
    ...
    public void setProgress(int progress) {
        if (mCurSegmentProgress != null) {
            mCurSegmentProgress.setEndTime(progress);
        }
    ...
    public void addSegment(long endFrameTime) {
        mCurSegmentProgress = new SegmentProgress();
        mCurSegmentProgress.setStartTime(endFrameTime);
        mCurSegmentProgress.setEndTime(endFrameTime);
        mSegmentProgressList.add(mCurSegmentProgress);
    }

    public void cleanSegment() {
        mSegmentProgressList.clear();
        mCurSegmentProgress = new SegmentProgress();
        mSegmentProgressList.add(mCurSegmentProgress);
        postInvalidate();
    }
    ...
    public void removeSegment(int index) {
        if (mSegmentProgressList.size() > index && index >= 0) {
            mSegmentProgressList.remove(index);
            if (!mSegmentProgressList.isEmpty()) {
                mCurSegmentProgress = mSegmentProgressList.get(mSegmentProgressList.size() - 1);
                long lastEndTime = getLastEndTime();
                mCurSegmentProgress.setStartTime(lastEndTime);
                mCurSegmentProgress.setEndTime(lastEndTime);
            } else {
                mCurSegmentProgress = new SegmentProgress();
                mSegmentProgressList.add(mCurSegmentProgress);
            }
    ...
    private long getLastEndTime() {
        int size = mSegmentProgressList.size();
        if (size >= 2) {//最后一个永远都是提前创建的，所以索引-2
            return mSegmentProgressList.get(size - 2).getEndTime();
        }
        return 0;
    }
    ...
    private static class SegmentProgress {
        long startTime = 0;
        long endTime = 0;
